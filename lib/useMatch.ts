...
import { notePlaytest } from "./playtest";
...
export function useSoloMatch(settings: SoloSettings): MatchController {
  ...
  const persist = useCallback((match: MatchState) => {
    if (brainRef.current && !saveSoloBattle(match, brainRef.current)) {
      ...
      notePlaytest("solo-save-unavailable");
    }
  }, []);
  ...
  useEffect(() => {
    if (settings.resume) {
      ...
      notePlaytest("solo-resumed", { round: saved.state.players.host.round, phase: saved.state.players.host.phase });
    } else start();
  ...
  const act = useCallback(
    (action: MatchAction) => {
      ...
      persist(match);
      notePlaytest("solo-action", { round: match.players.host.round, phase: match.players.host.phase, action: action.type });
      setState(structuredClone(match));
      lockRef.current.release();
      pumpEnemy();
    },
...
export function useRoomMatch(matchId: string | null): MatchController {
  ...
  useEffect(() => {
    ...
    const attempt = async () => {
      ...
      try {
        ...
        setError(null);
        notePlaytest("move-confirmed", { action: pending.action.type, round: pending.round });
      } catch (reason) {
        if (disposed || pendingRef.current?.id !== pending.id) return;
        if (isTransientRoomError(reason)) {
          setRecoveringMove(true);
          notePlaytest("move-checking", { action: pending.action.type, round: pending.round });
  ...
  const act = useCallback((action: MatchAction) => {
    ...
    setBusy(true);
    notePlaytest("move-sent", { action: action.type, round: you.round, phase: you.phase });
    retryMoveRef.current();
  }, [matchId]);
